...
Open `src/main.ts` (or `src/main.js` for vanilla JS projects) and remove all the autogenerated code. Then add the snippets below.

If you prefer to skip the step-by-step walkthrough, jump to the [full version](#full-version).
